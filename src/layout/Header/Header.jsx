...
import { useNavbar } from '../../contexts/NavbarContext'; // Context'i import et
...
const Header = () => {
  ...
  return (
    <div className={`header-container ${isNavbarCollapsed ? 'collapsed' : ''}`}>
      ...
      <div className={`logout-section ${isNavbarCollapsed ? 'collapsed' : ''}`}>
        <Link to="/logout">
          <img src={LogoutIcon} alt="Logout" className={`logout-icon ${isNavbarCollapsed ? 'collapsed' : ''}`} />
